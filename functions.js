...
return channel.videos.some(cv => cv.title === videoTitle);
}
// console.log(channelHasVideo("Joe Rogan Experience #1470 - Elon Musk", channels[0]));
...
function getChannelByName(channelName, channels) {
  return channels.find(cv => cv.name === channelName);
}
...
function searchChannels(query, channels) {
  return channels.filter(cv => cv.name.includes(query) || cv.description.includes(query));
}
